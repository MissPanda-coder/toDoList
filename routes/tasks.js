...
// Middleware pour les réponses réussies
...
router.post('/', (req, res) => {
    ...
    db.query(query, [taskTitle, taskContent, isFinished, status_idstatus], (err, results) => {
        if (err) {
            ...
            res.json({ message: "Task successfully added", id: results.insertId });
        }
    });
});
...
router.delete('/:id', (req, res) => {
    ...
    db.query('DELETE * FROM task WHERE idtask = ?', [id], (err, results) => {
        if (err) {
            ...
            res.status(201).json({ message: "Task successfully deleted", id: id });
        }
    });
});
